...
const filteredSameStops = computed(() => {
  if (isAciveAllFilter.value) {
    ...
    return userStore.ticketsList.filter(
      (el) => el.originStops === el.departureStops
    );
  }
});
...
const filteredByStops = computed(() => {
  ...
  return filteredSameStops.value.filter((el) =>
    userStore.userFilter.includes(el.originStops)
  );
});
...
const filterByDuration = computed(() => {
  return [...filteredByStops.value].sort(
    (a, b) => a.durationFly - b.durationFly
  );
});
...
const hiddenBtn = computed(() => {
  return (
    filteredTicker.value &&
    filteredTicker.value.length &&
    filterByTopNav.value.length > countList
  );
});
...
const disabledBtn = computed(() => {
  return (
    limitCard.value >=
    (stateFilter.value
      ? filterByTopNav.value.length - 1
      : userStore.ticketsList.length)
  );
});
  ...
  <section class="main-section" aria-label="Основний контент">
    ...
    <div class="container flex-container">
      ...
      <div class="main-list-container">
        ...
        <ul
          class="main-list-card"
          v-if="filteredTicker && filteredTicker.length"
        >
          <li
            class="main-card-item"
            v-for="card in filteredTicker"
            :key="card.keyId"
          >
            <CardComponent
        ...
        <div class="preload-wrapper" v-else>
          <ul class="main-list-card" v-if="userStore.finishDispatch">
            <li
              class="main-card-item relative-main-card"
